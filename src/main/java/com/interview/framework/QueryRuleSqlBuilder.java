...
import com.interview.framework.QueryRule.Rule;
...
public class QueryRuleSqlBuilder {
	...
	private List<String> groupProperties; // 保存分组参数
	private GroupBy groupBy; // 保存分组参数以及分组函数信息等
	private Limit limit; // 分页相关
	
	private String whereSql = ""; 
	private String orderSql = "";
	private String groupSql = "";
	private String limitSql = "";
	private String groupBySelectSql = "";
	 ...
	 * @return
	 */
	public String getGroupBySelectSql() {
		return groupBySelectSql;
	}
	...
	public QueryRuleSqlBuilder(QueryRule queryRule) {
		...
		groupProperties = new ArrayList<String>();
		limit = new Limit();
		List<GroupFunction> groupFunctionList  = new ArrayList<>();
		groupBy = new GroupBy(groupFunctionList,groupProperties);
		for (Rule rule : queryRule.getRuleList()) {
			...
			case QueryRule.DESC_ORDER:
				processOrder(rule);
				break;
			case QueryRule.LIMIT:
				processLimit(rule);
				break;
			case QueryRule.GROUP:
				processGroup(rule);
				break;
			case QueryRule.GROUP_COUNT:
				processGroupCount(rule);
				break;
			case QueryRule.GROUP_SUM:
				processGroupSum(rule);
				break;
			default:
	 ...
	 * @return
	 */
	protected String removeGroups(String sql) {
	 ...
	 * @return
	 */
	protected String removeLimit(String sql) {
		if(sql.toLowerCase().matches("limit\\s+")){
			int beginPos = sql.toLowerCase().indexOf("limit");
			return sql.substring(beginPos);
		}else{
			return sql;
		}
	...
	private void processLimit(Rule rule) {
		if(Objects.equals(rule.getType(),QueryRule.LIMIT)) {
			limit = rule.getLimit();
		}
	}

	private void processGroup(Rule rule) {
		if(Objects.equals(rule.getType(),QueryRule.GROUP)) {
			for(String groupStr : rule.getGroupProperties()) {
				groupProperties.add(groupStr);
			}
		}
	}

	private void processGroupSum(Rule rule) {
		if(Objects.equals(rule.getType(),QueryRule.GROUP_SUM)) {
			GroupFunction groupFunction = new GroupFunction(QueryRule.GROUP_SUM,rule.getPropertyName());
			groupBy.getGroupFunctionList().add(groupFunction);
		}
	}

	private void processGroupCount(Rule rule) {
		if(Objects.equals(rule.getType(),QueryRule.GROUP_COUNT)) {
			GroupFunction groupFunction = new GroupFunction(QueryRule.GROUP_COUNT,rule.getPropertyName());
			groupBy.getGroupFunctionList().add(groupFunction);
		}
	}
	 ...
	 * 拼装分组语句
	 */
	private void appendGroupSql(){
		StringBuffer groupSql = new StringBuffer();
		for (int i = 0 ; i < groupProperties.size(); i ++) {
			if(i > 0 && i < groupProperties.size()){
				groupSql.append(",");
			}
			groupSql.append(groupProperties.get(i).toString());
		}
		this.groupSql = removeSelect(removeGroups(groupSql.toString()));
	}

	/**
	 * 拼装分页语句
	 */
	private void appendLimitSql(){
		StringBuffer limitSql = new StringBuffer();
		if(limit != null) {
			limitSql.append(limit.getPageIndex()).append(",").append(limit.getPageSize());
			this.limitSql = removeSelect(removeLimit(limitSql.toString()));
		}
	}
	 ...
	 * 拼装select中分组函数
	 */
	private void appendSelectGroupSql(){
		if(groupBy != null) {
			...
			if(groupFunctionList != null && groupFunctionList.size() > 0) {
				for(GroupFunction groupFunction : groupFunctionList) {
					switch (groupFunction.getFunctionName()) {
						case QueryRule.GROUP_COUNT:
							groupSql.add("count("+ groupFunction.getPropertyName() + ") as count"+captureName(groupFunction.getPropertyName()));
							break;
						case QueryRule.GROUP_SUM:
							groupSql.add("sum("+ groupFunction.getPropertyName() + ") as sum"+captureName(groupFunction.getPropertyName()));
							break;
						default:
							break;
					}
				}
				this.groupBySelectSql = groupSql.stream().collect(Collectors.joining(","));
			}
		}
	}
